...
std::streamsize LocalFileHeader::read(scopedistream* source) {
    ...
    try {
        bytes_read += read_record(source, 0, &record, LFH_SIZE, LFH_SIGN);
    } catch (signature_does_not_match& e) {
        throw bad_zip_file("Local file header " + std::string(e.what()));
    } catch (read_error& e) {
        throw bad_zip_file("Local file header read error. " +
                           std::string(e.what()));
    }
...
std::streamsize LocalFileHeader::read_extra_field(scopedistream* source,
